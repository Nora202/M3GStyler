...
class MulLayer(nn.Module):
    ...
    def forward(self, cF, sF, clip_feature, train_step=0,text_inference=False):
        '''
        input:
        ...
        if text_inference==True:
            ...
            if train_step == 0:
                ...
                traj = sample_ode(flow_model, z0=flow_begin_embedding, N=self.N)
        ...
        if self.adain:
            ...
            if train_step==0:
                ...
                traj = sample_ode(flow_model, z0=flow_begin_embedding, N=self.N)
                flow_end_pred = traj[-1]
                loss1 = self.dis_loss(pred, target)
                loss2 = self.dis_loss(flow_begin_embedding, flow_end_pred)
                loss = loss1 + loss2
                cF = cF*(flow_end_pred[1:2, :].T) + (flow_end_pred[0:1, :].T)
